    ...
    <div className="flex flex-col border border-primary-800 md:flex-row">
      ...
      <div className="flex flex-grow flex-col px-6 py-3">
        ...
        <div className="mt-auto flex flex-wrap items-baseline gap-5">
          ...
          <p className="mx-auto text-sm text-primary-400 md:mx-auto md:mr-0">
            Booked {format(new Date(created_at), "EEE, MMM dd yyyy, p")}
          </p>
        </div>
      </div>

      <div className="flex flex-row justify-between border-l border-primary-800 md:w-[100px] md:flex-col">
        {!isPast(startDate) ? (
          <>
            <Link
              href={`/account/reservations/edit/${id}`}
              className="group flex flex-grow items-center justify-center gap-2 border-b border-primary-800 px-3 py-4 text-xs font-bold uppercase text-primary-300 transition-colors hover:bg-accent-600 hover:text-primary-900 md:justify-start md:py-0"
            >
